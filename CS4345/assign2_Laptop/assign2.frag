...
#version 110 core // GLSL 1.1, OpenGL 2.0, helt sjukt att vi kör med dessa! :/
...
void main()
{
    float P1 = texture2DRect( InputTex, gl_FragCoord.xy ).r; // Read from red channel, 0 = texA, 1 = texB
    float P2;
    ...
    if ( PassCount % 2 == 0 )  // PassCount is Even.
    {
		...
		if (col%2 == 0)
		{
			...
			P2 = texture2DRect( InputTex, lookupVec ).r;
