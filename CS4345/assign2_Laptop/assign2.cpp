...
// COMMENTS TO GRADER:
// Skillnaden mellan den här och MediaLab2A är att den här är byggd i VS2015 istället för VS2008/VS2013 
// och använder sig utav GL_RED istället för GL_ALPHA, som är depricated...
//
//============================================================
//
// FILE: assign2.cpp
...
static const int texWidth = 200; //200;  // Always an even number.
static const int texHeight = 125; // 125;
...
//-----------------------------------------------------------------------------
	...
	// OBS!!!!
	// I changed GL_ALPHA32F_ARB (depricated) to GL_R32F and GL_ALPHA to GL_RED
    ...
    glTexImage2D( GL_TEXTURE_RECTANGLE_ARB, 0, GL_R32F,
                  texWidth, texHeight, 0, GL_RED, GL_FLOAT, inputArray );
    printOpenGLError();
    ...
    glTexParameteri( GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_WRAP_T, GL_CLAMP );
    glTexImage2D( GL_TEXTURE_RECTANGLE_ARB, 0, GL_R32F,
                  texWidth, texHeight, 0, GL_RED, GL_FLOAT, NULL );
    printOpenGLError();
...
//-----------------------------------------------------------------------------
	...
	// Why do we start with the wrong numbers?
	// Edit: because we start by switching instead of starting by rendering in ping-pong...
    ...
    //---------------------------------------------------------
	    ...
	    //*********** EDITED **************
		//glPolygonMode(GL_FRONT, GL_FILL); // Already defined above (line 191)
		glBegin(GL_QUADS);
			glVertex2f(0, 0);
...
//-----------------------------------------------------------------------------
	...
	// OBS!! GL_ALPHA changed to GL_RED
	// It doesn't really make any difference but it's nice with the same convention
    glReadPixels( 0, 0, texWidth, texHeight, GL_RED, GL_FLOAT, outputArray );
    printOpenGLError();
