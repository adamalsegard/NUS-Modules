...
void MakeReflectionImage( void )
{
	...
	//printf("reflPos[0]: %d , reflPos[1]: %d , reflPos[2]: %d \n", reflPos[0], reflPos[1], reflPos[2]);
...
void MyDisplay( void )
{
	...
	// Create reflection image before shadows so we can draw table later
	MakeReflectionImage(); // Skip for the moment
...
void DrawCube(void)
{
	...
	glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
	glBindTexture(GL_TEXTURE_2D, containerTexObj); // containerTexObj
