...
int size;
// Use blkSize 32 to best utilize the threads. Warp size: 32. Maximum threads in block = 1024 (32x32)
...
*/	 
	...
	//printf("New Size: %1i", newSize);
	
	
	// Each thread calculates one value for the resulting matrix
	float tmpRes = 0;
	...
	{			
		...
		// Blocks from A moves horizontally, B vertically
...
void work()
{
	...
	dim = (size%BLOCK_SIZE==0) ? size/BLOCK_SIZE : size/BLOCK_SIZE + 1; //ceil(double(size)/double(BLOCK_SIZE)); 	// Just easier to read
	dim3 blocksPerGrid(dim, dim);			// a grid of CUDA thread blocks, covers all elements in matrices
	
	//int newSize = dim*BLOCK_SIZE;
	//init_zero_matrix(a, newSize);
	//init_zero_matrix(b, newSize);
