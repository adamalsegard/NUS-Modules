...
int main( int argc, char** argv){
    ...
    if (orig_rank == MASTER_ID){
        ...
        if(orig_rank < 4){
            MPI_Irecv(patterns[N][0], pMatSize, MPI_CHAR, MASTER_ID, orig_rank, MPI_COMM_WORLD, &reqs[orig_rank+5]);
            //MPI_Get_count(&Stat, MPI_CHAR, &count);
            //printf("Processor: %i received %i chars for Pattern from Master node \n", orig_rank, count);
            
            MPI_Irecv(curW[0], wMatSize, MPI_CHAR, MASTER_ID, orig_rank*2, MPI_COMM_WORLD, &reqs[orig_rank+8]);
            //MPI_Get_count(&Stat, MPI_CHAR, &count);
            //printf("Processor: %i received %i chars for World from Master node \n", orig_rank, count);
...
*/
    ...
    for (iter = 0; iter < iterations; iter++){
        ...
        // Send back to GLOBAL master, also blocking (for correct order)
        if (orig_rank == MASTER_ID){
